...
def submit_attendance(entry):
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            send_telegram_message(telegram_bot_token, telegram_chat_id, f"Absen Sore bos {name} telah dilaksanakan.")
            print(f"Absen Sore bos {name} telah dilaksanakan.")
            ...
            if attempt < max_retries - 1:
                ...
                send_telegram_message(telegram_bot_token, telegram_chat_id, f"Absen Spre bos {name} Gagal dilaksanakan.")
                print(error_message)
        finally:
            if driver:
                driver.quit()
...
def run_scheduling():
    start_time = config_list[0]['sore']  # Assuming all entries have the same start time
