...
def submit_attendance(entry):
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            send_telegram_message(telegram_bot_token, telegram_chat_id, f"Absen Siang bos {name} telah dilaksanakan.")
            print(f"Absen Siang bos {name} telah dilaksanakan.")
            ...
            if attempt < max_retries - 1:
                ...
                error_message = f"Attendance submission failed for {name} after {max_retries} attempts at {datetime.now().strftime('%Y-%m-%d %H:%M')}"
                send_telegram_message(telegram_bot_token, telegram_chat_id, f"Absen Siang bos {name} Gagal dilaksanakan.")
                print(error_message)
        finally:
            if driver:
                driver.quit()
...
def run_scheduling():
    start_time = config_list[0]['siang']  # Assuming all entries have the same start time
    allowed_days = [0, 1, 2, 3, 4]  # Monday, Wednesday, Friday (0 = Monday, 6 = Sunday)
